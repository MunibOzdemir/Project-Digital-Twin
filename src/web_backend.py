# web_backend.py - Complete backend with heat risk visualization
import json
import os
import hashlib
import pickle
...
def create_water_mask(image_shape, bounds, water_gdf):
    ...
    print(
        f"   Water mask: {water_pixels}/{total_pixels} pixels ({water_pixels/total_pixels*100:.1f}%)"
    )
    ...
    for check in range(max_checks):
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if status == "COMPLETED":
                    download_link = (
                        status_data.get("_links", {}).get("download", {}).get("href")
                    )
                    if download_link:
                    ...
                    elapsed = (check + 1) * 30
                    print(
                        f"   Status: {status} (HTTP {response.status_code}) (elapsed: {elapsed//60}m {elapsed%60}s)"
                    )
                    time.sleep(30)
...
def load_heat_risk_data():
    """
    Load heat risk GeoJSON data for visualization
    Returns:
        GeoDataFrame with heat risk data
    """
    # Use the original scenario file
    filename = "risk_original_scenario.geojson"
    file_path = DATA_DIR / filename

    if not file_path.exists():
        print(f"Heat risk file not found: {file_path}")
        return None

    try:
        print(f"Loading heat risk data from: {file_path}")
        ...
        if "risk_per_1000_inhabitants" in gdf.columns:
            risk_vals = gdf["risk_per_1000_inhabitants"].dropna()
            if len(risk_vals) > 0:
                print(f"   Risk range: {risk_vals.min():.4f} - {risk_vals.max():.4f}")
                print(f"   Mean risk: {risk_vals.mean():.4f}")

        return gdf

    except Exception as e:
        print(f"Error loading heat risk data: {e}")
        import traceback
...
def create_heat_risk_visualization(opacity=0.8):
    """
    Create heat map visualization for heat risk data using direct polygon rendering
    """
    try:
        # Load the GeoJSON data
        gdf = load_heat_risk_data()

        if gdf is None:
            print(f"Failed to load heat risk data")
            return None, None

        print(f"\n=== Creating heat risk visualization ===")
        print(f"Loaded {len(gdf)} features")

        # Check if risk column exists
        if "risk_per_1000_inhabitants" not in gdf.columns:
            print(f"Risk column not found. Available: {gdf.columns.tolist()}")
            return None, None

        # Get risk values and ensure they're numeric
        gdf["risk_value"] = pd.to_numeric(
            gdf["risk_per_1000_inhabitants"], errors="coerce"
        )
        gdf = gdf.dropna(subset=["risk_value"])

        if len(gdf) == 0:
            print("No valid risk values after cleaning")
            return None, None

        # Get value range
        vmin = gdf["risk_value"].min()
        vmax = gdf["risk_value"].max()

        print(f"Risk range: {vmin:.6f} to {vmax:.6f}")
        print(f"Mean risk: {gdf['risk_value'].mean():.6f}")
        print(f"Std dev: {gdf['risk_value'].std():.6f}")

        # Create figure using matplotlib
        fig, ax = plt.subplots(1, 1, figsize=(10, 10))

        # Plot the GeoDataFrame with black edges
        gdf.plot(
            column="risk_value",
            cmap="RdYlGn_r",
            ax=ax,
        ...
        stats = {
            "min_risk": float(vmin),
            "max_risk": float(vmax),
            "mean_risk": float(gdf["risk_value"].mean()),
            "std_risk": float(gdf["risk_value"].std()),
            "total_areas": len(gdf),
        }

        print(f"Successfully created heat risk visualization")
        print(f"Image size: {len(img_base64)} bytes (base64)")

        return img_base64, stats

    except Exception as e:
        print(f"Error in create_heat_risk_visualization: {e}")
        import traceback
...
if ensure_bgt_data_available():
    ...
    print(
        "Continuing without BGT data - illegal construction detection will use radar only"
    )
    BGT_DATA = None
    ...
    try:
        ...
        if include_bgt and BGT_DATA is not None:
            try:
                ...
                print(
                    f"Added BGT overlay to RGB change detection: {np.sum(bgt_mask)} pixels"
                )
        ...
        if bgt_mask is not None:
            ...
            rgba[changes_without_bgt, 0] = (
                255  # Full red intensity for better visibility
            )
...
def fetch_sentinel1_product(product_type, start_date, orbit_direction="DESCENDING"):
    ...
    try:
        ...
        print(
            f"   Shape: {arr.shape}, Range: [{np.nanmin(arr):.3f}, {np.nanmax(arr):.3f}]"
        )
...
def detect_radar_changes(date1="2019-06-01", date2="2022-06-01", threshold=0.05):
    ...
    try:
        ...
        ref_data = fetch_sentinel1_product(
            product_type="VV", start_date=date1, orbit_direction="DESCENDING"
        )

        print("Fetching comparison radar data...")
        comp_data = fetch_sentinel1_product(
            product_type="VV", start_date=date2, orbit_direction="DESCENDING"
        )
        ...
        if product_type == "RGB":
            ...
            if data.ndim == 3:
                if data.shape[2] == 4:
                    ...
                    alpha = np.where(alpha_raw > 0.1, int(255 * opacity), 0).astype(
                        np.uint8
                    )
                    ...
                    alpha = np.where(
                        is_black | is_nodata, 0, int(255 * opacity)
                    ).astype(np.uint8)
            ...
            if data.ndim == 3:
                if data.shape[2] >= 2:
                    ...
                    if np.any(np.isnan(index_data)):
                        ...
                        alpha = (
                            alpha_raw > 0.1
                        ) * opacity  # Convert to float mask with opacity
                else:
...
def get_satellite_data():
    ...
    try:
        ...
        if not date:
            return (
                jsonify({"success": False, "error": "Date parameter is required"}),
                400,
            )
        ...
        if cached_data is not None:
            ...
            satellite_data = fetch_sentinel_data(
                product_type, date, max_cloud, apply_water_mask
            )
        ...
        image_b64 = array_to_base64_image(
            satellite_data, product_type=product_type, opacity=opacity
        )
        ...
        return jsonify(
            {
                ...
                and product_type == "NDCI"
                and WATER_BODIES is not None,
            }
        )
...
def get_illegal_construction():
    ...
    try:
        ...
        if results is None:
            return (
                jsonify(
                    {"success": False, "error": "Failed to detect illegal construction"}
                ),
                500,
            )
        ...
        if image_b64 is None:
            return (
                jsonify({"success": False, "error": "Failed to create visualization"}),
                500,
            )
...
def get_rgb_change_detection():
    ...
    try:
        ...
        if results is None:
            return (
                jsonify({"success": False, "error": "Failed to detect RGB changes"}),
                500,
            )
...
def get_heat_risk():
    """Get heat risk visualization"""
    try:
        data = request.get_json()
        opacity = data.get("opacity", 0.8)

        print(f"\n=== HEAT RISK API CALLED ===")
        print(f"Opacity: {opacity}")

        # Create visualization
        image_b64, stats = create_heat_risk_visualization(opacity)
        ...
        if image_b64 is None:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": f"Failed to create heat risk visualization",
                    }
                ),
                500,
            )

        print(f"Generated image size: {len(image_b64)} bytes")
        ...
        response_data = {
            ...
            "statistics": stats,
            "title": "Heat Risk Map",
            "description": "Risico overleiden ouderen per 1000 inwoners",
        ...
        print(f"Error in heat risk endpoint: {e}")
        import traceback
...
def get_bgt_layer():
    ...
    try:
        ...
        if image_b64 is None:
            return (
                jsonify(
                    {"success": False, "error": "Failed to create BGT visualization"}
                ),
                500,
            )
    ...
    return jsonify(
        {
            ...
            "bounds_source": (
                "alkmaar.geojson"
                if (DATA_DIR / "alkmaar.geojson").exists()
                else "default"
            ),
            "geometry_type": ALKMAAR_GEOMETRY.get("type", "unknown"),
            "cache_files": (
                len(list(CACHE_DIR.glob("*.pkl"))) if CACHE_DIR.exists() else 0
            ),
            "area_km2": round(
                (ALKMAAR_BOUNDS[1][1] - ALKMAAR_BOUNDS[0][1])
                * (ALKMAAR_BOUNDS[1][0] - ALKMAAR_BOUNDS[0][0])
                * 111
                * 111,
                1,
            ),
...
def main(port=8505, debug=True):
    ...
    if cache_dir.exists():
        print(f"Cache directory found: {cache_dir.absolute()}")

        # Count cache files
        cache_files = list(cache_dir.glob("*.pkl"))
        print(f"Found {len(cache_files)} cache files")

        # Look for heat risk cache files
        heat_risk_cache = [f for f in cache_files if "heat_risk" in f.name]
        if heat_risk_cache:
            print(f"\nFound {len(heat_risk_cache)} heat risk cache files:")
            for f in heat_risk_cache:
                print(f"  - {f.name}")
                try:
                    os.remove(f)
                    print(f"    ✓ Deleted")
    ...
    print("\nPlease restart your Flask server and try both scenarios again")
    ...
    print(f"   POST /api/heat-risk         - Heat risk visualization")
    print(f"   GET  /api/bgt-layer         - BGT layer visualization")
    ...
    print(f"   • Heat risk maps for original and +10% green scenarios")
