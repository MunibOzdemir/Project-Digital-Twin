...
df_alkmaar_multi = df_cbs_multi[(df_cbs_multi['gm_naam'] == "Alkmaar") & (df_cbs_multi['recs'] == 'Buurt')].copy()
...
df_alkmaar_multi['percent_old'] = (df_alkmaar_multi['a_65_oo'] / df_alkmaar_multi['a_inw']) * 100
...
expected_rows = df_alkmaar_multi['regio'].nunique() * len(df_alkmaar_multi['data_year'].unique())
...
df_hot_days = df_knmi[df_knmi["temp_max"] > 27.0].groupby(df_knmi["date"].dt.year).size().reset_index(name="hot_days")

# Merge hot days data with CBS data
df_alkmaar_multi = df_alkmaar_multi.merge(df_hot_days, left_on="data_year", right_on="date", how="left")
...
def clean_and_match_names(df_alkmaar, top10nl_green_results_fixed):
    ...
    for alkmaar_name in alkmaar_names:
        ...
        for wijk_name in wijk_names:
            ...
            if normalized_alkmaar in normalized_wijk or normalized_wijk in normalized_alkmaar:
                score = min(len(normalized_alkmaar), len(normalized_wijk)) / max(len(normalized_alkmaar), len(normalized_wijk))
                if score > best_score:
                    best_score = score
        ...
        if best_match and best_score > 0.5:  # Only accept matches with >50% confidence
            ...
            fuzzy_matches += 1
            print(f"   🔗 Fuzzy match: '{alkmaar_name}' → '{best_match}' (score: {best_score:.2f})")
        else:
            no_matches.append(alkmaar_name)
            print(f"   ❌ No match found: '{alkmaar_name}'")
    
    # Apply the mapping
    df_alkmaar_clean['regio'] = df_alkmaar_clean['regio'].map(name_mapping).fillna(df_alkmaar_clean['regio'])
...
def merge_datasets_with_matching(df_alkmaar, top10nl_green_results_fixed):
    ...
    df_alkmaar_matched, mapping = clean_and_match_names(df_alkmaar, top10nl_green_results_fixed)
    ...
    if sum(merged_df['_merge'] == 'right_only') > 0:
        ...
        unmatched_green = merged_df[merged_df['_merge'] == 'right_only']['wijk_name'].unique()
...
merged_data_multi_clean, merged_data_multi_full = merge_datasets_with_matching(df_alkmaar_multi, top10nl_green_results_fixed)
...
if len(merged_data_multi_clean) > 0:
    print(f"\nFirst few rows:")
    print(merged_data_multi_clean[['regio', 'wijk_name', 'green_percentage', 'a_65_oo', 'p_ste']].head())
else:
...
#training data
training_data = merged_data_multi_clean[merged_data_multi_clean['data_year'].isin([2019, 2022, 2023])]
...
X_multi = pd.DataFrame({
    ...
    'hot_days_x_percent_old': training_data['hot_days'] * training_data['percent_old'],
    'hot_days_x_green': training_data['hot_days'] * training_data['green_percentage'],
